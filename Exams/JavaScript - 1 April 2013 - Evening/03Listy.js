/**
 *Bai Ivan also known as @bivan27 is a famous blogger and developer. Because he is Bulgarian, he is not a big fan of buying software, so he doesnít have a legal version of Microsoft Office. Also because of his neighbour who doesnít like him and sends him bad boys from GDBOP(√ƒ¡Œœ) every week to check his PC for illegal software, @bivan27 canít use Excel.
 One day @bivan27 decided to make his own programming language called Listy which will do Excelís calculations with a list of numbers. His conception for now is very simple. He can assign lists to a variables, get the min or max value from a list, get average or sum the elements of the list. Each of the functions gets as parameters list of numbers or variables in square brackets.îdefî is a keyword used to define a variable. Here are some syntax examples.
...
function solve(params)
{
    ...
    while(true)
    {
        ...
        var line = params[i].trim();
        var list = line.split('[')[1].split(',');
            list = list.map(function(lstItem){return lstItem.trimBracket().trim()});
        var def = line.split('[')[0].split(' ').map(function(item){return item.trim();}).filter(function(n){return !!n;});
        if(def.length<2)
        {
            ...
            if(def.length===1)
            {
                ...
                operation = def[0];
                return solveOperation(operation,list,variables);
            }
               return solveOperation(operation,list,variables).join(', ');
            break;
        }else
        {
            name = def[1];
            operation = def[2];
            variables[name] = solveOperation(operation,list,variables);
            // call function to solve operation
        }
        i+=1;

    }

        function solveOperation(operation,list,model)
        {
            var sum = 0;
            var avg = 0;
            ...
            for(var i =0;i<list.length;i+=1)
            {
                ...
                if( num!==0 && !num)
                {
                    list.splice.apply(list,[i,1].concat(model[list[i]]));
